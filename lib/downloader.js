...
var findUndownloaded = function(connection, limit) {
    return new Promise(function(resolve, reject) {
        connection.query('SELECT * FROM sets WHERE is_downloaded=0 LIMIT ?', [limit], function(err, rows) {
            if (err) {
                reject(err);
            } else if (rows === 0) {
                reject("there are no undownloaded URLs");
            } else {
                resolve(rows);
            }
        });
...
var findUndownloadedPromise = findUndownloaded(connection, 1);

// findUndownloadedPromise.fail(function(err) {
//     console.log(err);
//     connection.end();
// });


findUndownloadedPromise.then(function(rows) {
    ...
    downloadPermalink(soundcloudUrl, function(data, duration) {
        ...
        if (data) {
            ...
            connection.query('UPDATE sets SET is_downloaded=1, duration=? WHERE url=?', [duration, soundcloudUrl], function(err) {
                console.log(err)
            });
